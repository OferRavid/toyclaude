...
def call_function(function_call_part, verbose=False):
    ...
    if function_name not in valid_function_calls:
    ...
    function_call_response = valid_function_calls[function_name](**function_args)
    return types.Content(
        role="tool",
        parts=[
            types.Part.from_function_response(
                name=function_name,
                response={"result": function_call_response},
            )
        ],
    )
